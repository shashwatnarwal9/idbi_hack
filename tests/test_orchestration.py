"""आय·AI Stage 6 tests: DAG and compose wiring (text/YAML level — Airflow
itself only runs inside Docker, not on Windows)."""
...
TASK_IDS = ["bronze_ingest", "silver_transform", "gold_build",
            "ge_validation", "confidence_branch", "train_model", "skip_scoring"]

STAGE_IMPORTS = ["aayai.bronze.ingest", "aayai.silver.transform",
                 "aayai.gold.build", "aayai.validation.run", "aayai.model.train"]
...
def test_compose_shape():
    ...
    assert {"postgres", "airflow-init", "airflow-webserver",
            "airflow-scheduler"} <= set(services)
