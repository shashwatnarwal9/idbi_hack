"""आय·AI Stage 4 — Great Expectations validation.
...
BAND_RULES = {"high": {"months": 12, "pct": 0.90},
              "medium": {"months": 6, "pct": 0.85}}

GOLD_KEY_FIELDS = ["customer_id", "income_type", "true_monthly_income",
                   "investable_surplus", "risk_capacity", "region",
                   "months_history", "pct_categorized"]
...
def silver_gate_suite() -> gx.ExpectationSuite:
    ...
    for col in ("txn_id", "customer_id", "category", "direction",
                "amount", "is_income", "parse_confidence"):
        suite.add_expectation(E.ExpectColumnValuesToNotBeNull(column=col))
    suite.add_expectation(E.ExpectColumnValuesToBeBetween(
        column="parse_confidence", min_value=0.0, max_value=1.0))
    suite.add_expectation(E.ExpectColumnValuesToBeBetween(
        column="amount", min_value=0.0, strict_min=True))
    suite.add_expectation(E.ExpectColumnValuesToBeInSet(
        column="direction", value_set=["credit", "debit"]))
    suite.add_expectation(E.ExpectColumnValuesToBeInSet(
        column="category", value_set=SILVER_CATEGORIES))
    return suite
...
def gold_gate_suite() -> gx.ExpectationSuite:
    ...
    suite.add_expectation(E.ExpectColumnValuesToBeBetween(   # no negative income
        column="true_monthly_income", min_value=0.0, strict_min=True))
    suite.add_expectation(E.ExpectColumnValuesToBeBetween(
        column="investable_surplus",
        min_value=SURPLUS_BOUNDS[0], max_value=SURPLUS_BOUNDS[1]))
    return suite
...
def confidence_suite() -> gx.ExpectationSuite:
    ...
    for band, rule in BAND_RULES.items():
        suite.add_expectation(E.ExpectColumnValuesToBeBetween(
            column="months_history", min_value=rule["months"],
            meta={"tier": band, "reason": f"months_history >= {rule['months']}"}))
        suite.add_expectation(E.ExpectColumnValuesToBeBetween(
            column="pct_categorized", min_value=rule["pct"],
            meta={"tier": band, "reason": f"pct_categorized >= {rule['pct']}"}))
    return suite


def run_suite(context, name: str, suite: gx.ExpectationSuite,
              df: pd.DataFrame, complete: bool = False):
...
def print_gate(name: str, result) -> bool:
    ...
    print(f"[validation] {name}: {'PASS' if result.success else 'FAIL'} "
          f"({ok}/{len(result.results)} expectations)")
    for r in result.results:
        if not r.success:
            cfg = r.expectation_config
            print(f"  FAILED {cfg.type}({cfg.kwargs.get('column')}): "
                  f"{r.result.get('unexpected_count')} unexpected rows")
    return bool(result.success)


def derive_bands(result, gold_df: pd.DataFrame) -> tuple[dict, dict]:
    """GE unexpected lists -> per-customer band + human-readable reasons."""
    ...
    bands, reasons = {}, {}
    for cid, months, pct in zip(gold_df["customer_id"],
                                gold_df["months_history"],
                                gold_df["pct_categorized"]):
...
def write_bands(con: duckdb.DuckDBPyConnection, bands: dict) -> None:
    band_df = pd.DataFrame({"customer_id": list(bands),
                            "confidence_band": list(bands.values())})
    con.register("band_map", band_df)
    cols = [r[0] for r in con.execute(f"DESCRIBE SELECT * FROM {PROFILES_READ}").fetchall()]
...
def main() -> None:
    ...
    silver_df = con.execute(
        ...
        f"is_income, parse_confidence FROM {SILVER_READ}").df()
    gold_df = con.execute(f"SELECT * FROM {PROFILES_READ}").df()
    context = build_context()

    gates_ok = print_gate("silver gate",
                          run_suite(context, "silver", silver_gate_suite(), silver_df))
    gates_ok &= print_gate("gold gate",
                           run_suite(context, "gold", gold_gate_suite(), gold_df))

    conf_result = run_suite(context, "confidence", confidence_suite(),
                            gold_df, complete=True)
    ...
    print("[validation] confidence_band distribution: "
    ...
    for target in ("high", "low"):
        ...
        if cid is None:
            ...
            continue
        print(f"[validation] example {target.upper()}: {cid} -- "
              + "; ".join(reasons[cid]))
