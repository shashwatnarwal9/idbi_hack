"""आय·AI (AayAI) Stage-0 synthetic data generator.
...
MONTHS = [(y, m) for y in (2025, 2026) for m in range(1, 13)
          if not (y == 2026 and m > 6)]                      # 2025-01 .. 2026-06
N_MONTHS = len(MONTHS)
...
INCOME_CATEGORIES = {"SALARY", "PENSION", "BUSINESS_INCOME", "GIG_PAYOUT",
                     "RENT_INCOME", "FREELANCE", "INTEREST"}

FIRST = ["Aarav", "Vivaan", "Aditya", "Ananya", "Diya", "Ishaan", "Kavya",
         "Rohan", "Priya", "Neha", "Arjun", "Sneha", "Rahul", "Pooja", "Amit",
         "Sunita", "Vikram", "Meera", "Karan", "Divya"]
LAST = ["Sharma", "Verma", "Gupta", "Iyer", "Nair", "Patel", "Reddy", "Singh",
        "Khan", "Das", "Joshi", "Kulkarni", "Mehta", "Chopra", "Banerjee", "Rao"]
CITIES = ["Mumbai", "Delhi", "Bengaluru", "Hyderabad", "Chennai", "Kolkata",
          "Pune", "Ahmedabad", "Jaipur", "Lucknow", "Indore", "Kochi"]
EMPLOYERS = ["ACME TECHNOLOGIES PVT LTD", "BHARAT INFOTECH LTD",
             "SUNRISE TEXTILES", "NATIONAL RETAIL CORP", "MEDIPLUS HEALTHCARE",
             "OMEGA CONSULTING", "TATA STEEL LTD", "INFOSYS LTD",
             "GODREJ AND BOYCE", "ZENITH PHARMA"]
GIG_PLATFORMS = ["SWIGGY", "ZOMATO", "UBER INDIA", "OLA", "RAPIDO",
                 "URBANCOMPANY", "BLINKIT", "ZEPTO"]
SUPPLIERS = ["MAHALAXMI TRADERS", "SHREE ENTERPRISES", "KM DISTRIBUTORS",
             "ROYAL AGENCIES", "GANESH WHOLESALE", "APEX SUPPLIES"]
GROCERY = ["DMART", "BIGBASKET", "RELIANCE FRESH", "MORE SUPERMARKET",
           "LOCAL KIRANA STORE", "JIOMART"]
DINING = ["SWIGGY", "ZOMATO", "DOMINOS", "MCDONALDS", "HALDIRAMS",
...
def _handle(rng: random.Random, name: str) -> str:
    return name.split()[0].lower() + str(rng.randrange(10, 999)) + "@" + rng.choice(UPI_PSPS)


def _card(rng: random.Random) -> str:
    return "4" + str(rng.randrange(10**4, 10**5)) + "XXXXXX" + str(rng.randrange(1000, 9999))
...
def narration(rng: random.Random, cat: str, p: dict, y: int, m: int) -> str:
    ...
    if cat == "SALARY":
        ...
        s = rng.choice([f"PENSION-{mn}{yy}-CPAO TREASURY-{_ref(rng)}",
        ...
        s = rng.choice([f"IMPS/P2A/{_rrn(rng)}/{pf} PAYOUT/{_ref(rng)}",
        ...
        s = rng.choice([f"UPI/{_rrn(rng)}/{p['tenant']}/{_handle(rng, p['tenant'])}/RENT {mn}",
                        f"IMPS-P2A-{_rrn(rng)}-RENT FOR {mn}-{p['tenant']}"])
    elif cat == "FREELANCE":
        client = _person(rng)
        s = rng.choice([f"NEFT-{bank}000{rng.randrange(1000, 9999)}-{client}-PROJECT PAYMENT-{_ref(rng)}",
                        f"UPI/{_rrn(rng)}/{client}/{_handle(rng, client)}/FREELANCE WORK"])
    elif cat == "INTEREST":
        s = rng.choice([f"INT.PD:01-{m:02d}-{y} TO 30-{m:02d}-{y}",
                        f"SB INTEREST {mn} {yy}", "CREDIT INTEREST CAPITALISED"])
    elif cat == "FD_MATURITY":
        s = f"FD MATURITY PROCEEDS {rng.randrange(10**7, 10**8)}"
    elif cat == "REFUND":
        s = rng.choice([f"REFUND/{rng.choice(SHOPPING)}/ORDER {rng.randrange(10**6, 10**7)}",
                        f"UPI/REV/{_rrn(rng)}/{rng.choice(SHOPPING)}/REFUND"])
    elif cat == "SELF_TRANSFER_IN":
        s = rng.choice([f"NEFT-{bank}000{rng.randrange(1000, 9999)}-{p['name'].upper()}-SELF-{_ref(rng)}",
                        f"IMPS FROM OWN AC {_rrn(rng)}"])
    elif cat == "P2P_IN":
        who = _person(rng)
        s = f"UPI/{_rrn(rng)}/{who}/{_handle(rng, who)}/PAYMENT"
    elif cat == "RENT_PAID":
        s = rng.choice([f"UPI/{_rrn(rng)}/{p['landlord']}/{_handle(rng, p['landlord'])}/RENT {mn}",
                        f"IMPS-P2A-{_rrn(rng)}-{p['landlord']}-HOUSE RENT"])
    elif cat == "EMI":
        s = rng.choice([f"ACH-D-{p['lender']}-EMI{rng.randrange(10**5, 10**6)}",
                        f"NACH/{p['lender']}/LOAN EMI/{_ref(rng)}"])
    elif cat == "SIP_INVESTMENT":
        s = rng.choice([f"ACH-D-{p['amc']}-SIP", f"NACH/{p['amc']}/SIP {rng.randrange(1, 29):02d}{m:02d}{yy}"])
    elif cat == "INSURANCE":
        s = f"ACH-D-{p['insurer']}-PREMIUM {rng.randrange(10**7, 10**8)}"
    elif cat == "UTILITIES":
        u = rng.choice(UTILITIES)
        s = rng.choice([f"BBPS/{u}/BILL PAY/{rng.randrange(10**6, 10**7)}",
        ...
        s = rng.choice([f"UPI/P2M/{_rrn(rng)}/{st}/{_handle(rng, st)}/PAYMENT",
                        f"POS {_card(rng)} {st}"])
    elif cat == "DINING":
        s = f"UPI/P2M/{_rrn(rng)}/{rng.choice(DINING)}/PAYMENT FROM PH"
    elif cat == "SHOPPING":
        st = rng.choice(SHOPPING)
        s = rng.choice([f"UPI/P2M/{_rrn(rng)}/{st}/{_handle(rng, st)}/SHOPPING",
                        f"POS {_card(rng)} {st}"])
    elif cat == "FUEL":
        ...
        s = rng.choice([f"ATM-CASH-NFS/{bank} ATM/{p['city'].upper()}/{rng.randrange(10**5, 10**6)}",
                        f"ATM WDL {_rrn(rng)}"])
    elif cat == "P2P_OUT":
        who = _person(rng)
        s = f"UPI/{_rrn(rng)}/{who}/{_handle(rng, who)}/SENT"
    elif cat == "BUSINESS_EXPENSE":
        s = rng.choice([f"NEFT-{bank}000{rng.randrange(1000, 9999)}-{rng.choice(SUPPLIERS)}-PAYMENT-{_ref(rng)}",
...
def make_profile(rng: random.Random, idx: int) -> dict:
    ...
    p = {
        ...
        "open_date": date(rng.randint(2012, 2024), rng.randint(1, 12), rng.randint(1, 28)),
    ...
    if arch in ("SALARIED", "SALARIED_PLUS"):
        p["salary"] = round(math.exp(rng.uniform(math.log(28_000), math.log(200_000))), -3)
        p["exp_income"] = p["salary"]
        if arch == "SALARIED_PLUS":
            p["rent_in"] = round(rng.uniform(8_000, 35_000), -3)
            p["freelance_target"] = round(rng.uniform(5_000, 25_000), -3) if rng.random() < 0.6 else 0
        ...
        p["margin_income"] = round(math.exp(rng.uniform(math.log(35_000), math.log(250_000))), -3)
    ...
    p["spend_ratio"] = rng.uniform(*{"SALARIED": (0.50, 0.85), "SALARIED_PLUS": (0.45, 0.75),
                                     "GIG": (0.60, 0.95), "BUSINESS": (0.50, 0.90),
                                     "PENSIONER": (0.50, 0.90)}[arch])
    renter_p = {"SALARIED": 0.55, "SALARIED_PLUS": 0.20, "GIG": 0.55,
                "BUSINESS": 0.35, "PENSIONER": 0.10}[arch]
    p["rent_paid"] = round(rng.uniform(0.18, 0.28) * p["exp_income"], -2) if rng.random() < renter_p else 0
    p["emi"] = round(rng.uniform(0.08, 0.20) * p["exp_income"], -2) if rng.random() < (0.15 if arch == "PENSIONER" else 0.45) else 0
    p["sip"] = round(rng.uniform(0.05, 0.15) * p["exp_income"], -2) if rng.random() < 0.45 else 0
...
def _ts(rng: random.Random, y: int, m: int, day: int | None = None,
        hours: tuple[int, int] = (7, 22)) -> datetime:
    return datetime(y, m, day or rng.randint(1, 28), rng.randint(*hours),
...
def month_events(rng: random.Random, p: dict, y: int, m: int) -> list[tuple]:
    ...
    # --- income credits
    if arch in ("SALARIED", "SALARIED_PLUS"):
        ev.append((_ts(rng, y, m, min(28, p["salary_day"] + rng.randint(-1, 1)) or 1, (9, 19)),
                   "SALARY", "CREDIT", p["salary"]))
    if arch == "SALARIED_PLUS":
        ev.append((_ts(rng, y, m, rng.randint(2, 7)), "RENT_INCOME", "CREDIT", p["rent_in"]))
    ...
    if arch == "BUSINESS":
        receipts = _split(rng, p["margin_income"] / p["margin"], rng.randint(8, 25), lo=500)
        for amt in receipts:
        ...
        for amt in _split(rng, expense_total, rng.randint(2, 6), lo=1000):
            ev.append((_ts(rng, y, m, None, (10, 18)), "BUSINESS_EXPENSE", "DEBIT", amt))
    if arch == "PENSIONER":
        ev.append((_ts(rng, y, m, rng.randint(1, 2), (9, 12)), "PENSION", "CREDIT", p["pension"]))
        if m in (3, 6, 9, 12):
            ev.append((_ts(rng, y, m, 28), "INTEREST", "CREDIT",
                       round(p["fd_interest"] * rng.uniform(0.9, 1.1), 2)))
    if m in (3, 6, 9, 12):  # savings-account interest for everyone
        ev.append((_ts(rng, y, m, 28), "INTEREST", "CREDIT", round(rng.uniform(50, 800), 2)))

    # --- non-income noise credits (the traps for is_income detection)
    if rng.random() < 0.15:
        ev.append((_ts(rng, y, m), "REFUND", "CREDIT", round(rng.uniform(100, 3000), 2)))
    if rng.random() < 0.25:
        ev.append((_ts(rng, y, m), "P2P_IN", "CREDIT", round(rng.uniform(500, 5000), 2)))
    if rng.random() < (0.35 if arch == "SALARIED_PLUS" else 0.15):
        ev.append((_ts(rng, y, m), "SELF_TRANSFER_IN", "CREDIT", round(rng.uniform(5000, 50000), -2)))
    if rng.random() < 0.02:
        ev.append((_ts(rng, y, m), "FD_MATURITY", "CREDIT", round(rng.uniform(20000, 200000), -3)))

    # --- fixed debits
    for cat, amt, day in (("RENT_PAID", p["rent_paid"], rng.randint(1, 7)),
    ...
    ev.append((_ts(rng, y, m), "MOBILE_RECHARGE", "DEBIT", float(rng.choice([199, 239, 299, 479, 599, 999]))))
    ...
    for cat, frac, kmax in (("GROCERIES", 0.32, 6), ("DINING", 0.14, 6), ("SHOPPING", 0.18, 4),
                            ("FUEL", 0.10, 3), ("MEDICAL", 0.06, 2), ("ENTERTAINMENT", 0.06, 2),
                            ("CASH_WITHDRAWAL", 0.10, 2), ("P2P_OUT", 0.04, 2)):
        for amt in _split(rng, variable * frac * rng.uniform(0.6, 1.4), rng.randint(0, kmax)):
...
def generate_customer(rng: random.Random, idx: int) -> tuple[dict, list[dict]]:
    ...
    for ts, cat, ttype, amt in events:
        balance = round(balance + amt, 2) if ttype == "CREDIT" else round(balance - amt, 2)
    ...
    surplus_ratio = (true_income - personal_spend / N_MONTHS) / true_income if true_income > 0 else 0.0
    prospect = true_income >= 60_000 and surplus_ratio >= 0.25
    if rng.random() < 0.04:                       # label noise
        prospect = not prospect

    arch = p["archetype"]
    declared_factor = {"SALARIED": (0.90, 1.05), "SALARIED_PLUS": (0.90, 1.00),
                       "GIG": (0.20, 0.60), "BUSINESS": (0.30, 0.70),
                       "PENSIONER": (0.90, 1.10)}[arch]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="AayAI Stage-0 raw data generator")
    ...
    txn_cols = ["txn_id", "customer_id", "timestamp", "txn_type", "amount",
                "balance", "narration", "_true_category", "_is_income"]
    ...
    print(f"[AayAI stage-0] wrote {len(all_txns):,} transactions "
    ...
    print(f"[AayAI stage-0] good prospects: "
