"""आय·AI Gold evaluation — reads "_" columns (evaluation only).

Prints three archetype profiles side by side, then how well the reconstructed
true_monthly_income tracks the ground-truth _true_monthly_income: Pearson r,
MAE, bias (derived - truth) and MAPE, overall and per archetype.
"""
...
SHOW_FIELDS = ["customer_id", "region", "occupation_declared",
               "declared_monthly_income", "income_type", "true_monthly_income",
               "_true_monthly_income", "income_volatility",
               "avg_monthly_essentials", "total_emi", "total_sip",
               "investable_surplus", "surplus_stability", "savings_rate",
               "risk_capacity", "months_history", "pct_categorized"]
...
def evaluate(con: duckdb.DuckDBPyConnection | None = None) -> dict:
    ...
    return {"corr": corr, "mae": mae, "bias": bias, "mape": mape, "n": n,
...
def print_samples(con: duckdb.DuckDBPyConnection) -> None:
    ...
    for occ in ("SALARIED", "GIG", "BUSINESS"):
        cur = con.execute(
            ...
            f"ORDER BY customer_id LIMIT 1", [occ])
...
def main() -> None:
    ...
    print(f"  pearson r = {m['corr']:.4f}   MAE = Rs {m['mae']:,.0f}   "
    ...
    print("[gold eval] notes: gig/business use a p25 floor (conservative by "
