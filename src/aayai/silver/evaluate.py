"""Silver evaluation: derived categories against ground truth.

Evaluation code is the only part of the pipeline permitted to read "_" columns.
Prints overall category accuracy, per-category precision/recall/support, the
salary / gig_income / p2p_in confusion matrix (income vs non-income credits is
the hard part of narration parsing), and derived is_income vs _is_income.

Ground truth uses a richer taxonomy than silver's fixed 16 categories, so it
is folded onto the silver taxonomy first (e.g. PENSION -> salary, DINING ->
food). The fold defines the right answer in silver's vocabulary; the rules
never see it.
"""
...
TRUTH_TO_SILVER = {
    ...
    "REFUND": "p2p_in",
    "SELF_TRANSFER_IN": "p2p_in",
    "P2P_IN": "p2p_in",
    "FD_MATURITY": "p2p_in",
    "CASH_WITHDRAWAL": "atm",
    "P2P_OUT": "p2p_out",
    "BUSINESS_EXPENSE": "p2p_out",
}


def evaluate(con: duckdb.DuckDBPyConnection | None = None) -> dict:
    """Score derived categories against folded ground truth.

    Args:
        con: optional open DuckDB connection; a fresh one is created if omitted.

    Returns:
        Dict with overall accuracy, per_category (precision, recall, support),
        the salary/gig_income/p2p_in confusion counts, and is_income accuracy.
    """
    ...
    for truth_raw, derived, n in pairs:
        ...
        if expected in focus or derived in focus:
            confusion3[
                (
                    expected if expected in focus else "other",
                    derived if derived in focus else "other",
                )
            ] += n
    ...
    n_income_ok = con.execute(
        f"SELECT count(*) FROM {TXN_READ} WHERE is_income = _is_income"
    ).fetchone()[0]
    return {
...
def main() -> None:
    ...
    m = evaluate()
    print(
        f"[silver eval] overall category accuracy: {m['overall']:.2%} "
        f"({m['correct']:,} / {m['total']:,})"
    )

    print(
        f"[silver eval] per-category precision/recall "
    ...
    print(
        "[silver eval] salary / gig_income / p2p_in confusion "
    ...
    print(
        "[silver eval] note: rent credits are real income in ground truth but "
        "the fixed taxonomy scores them as 'rent' (not income) by design."
    )
    print(
        f"[silver eval] categories below 85% precision or recall: "
